...
# Security, Privacy & Clinical Use

* **Data privacy:** store and transmit identifiable genomic data securely (encrypted storage, access controls)
* **Regulatory:** this pipeline is research-use only unless validated and accredited locally. Do not use for clinical decision-making without laboratory validation and appropriate certifications.

# Development & Contribution

* Provide CONTRIBUTING.md template, issue and PR templates. Define coding style, tests, and code review process.

# Template files (to fill)

This repository includes a number of template files you should fill with lab-specific values. Below is a summary of templates included in this README; actual template files should be added to `templates/` in the repo.

1. `config.yml` — central configuration for references, resources, and filters (see above template snippet).
2. `samples.csv` — required sample sheet template (see above snippet).
3. `run-manifest.json` — run-level metadata template (auto-generated by pipeline when possible). Example fields:

   * run_id, date, user, pipeline_version, tool_versions, input_files, config_used
4. `clinical_filter.json` — canonical clinical filter settings for reviewers
5. `report_template.html` — HTML report skeleton (placeholders for metrics and tables)
6. `acmg_checklist.md` — ACMG criteria template to be filled for each candidate
7. `cnv_reference_list.txt` — list of normal samples used for CNV calling
8. `ci.yml` — GitHub Actions template for linting and basic tests
9. `Dockerfile` / `Singularity.def` — container build files (placeholders for exact versions)

# FAQ

**Q: Which reference should I use?**
A: GRCh38 is recommended (and used by this pipeline) but the pipeline supports alternate references via `config.yml`.

**Q: How do I tune filters?**
A: Filters depend on capture kit, read length, and platform. Use a small validation set to tune allele frequency and quality thresholds. Document changes in the `CHANGELOG`.

# License & Citation

* **License:** Add your chosen license here (e.g., MIT, Apache-2.0)
* **Citation:** Provide a citation line describing how to cite the pipeline in publications.

---

## Appendix: Useful command snippets & templates (fill-in)

### run-manifest.json example

```json
{
  "run_id": "RUN_2025_0001",
  "date": "2025-12-20",
  "user": "your.name",
...
```dockerfile
FROM ubuntu:20.04
# install dependencies, conda, and pipeline
# COPY pipeline /opt/wes-pipeline
# RUN conda env create -f /opt/wes-pipeline/environment.yml
```
...
1. Add exact tool versions and produce `environment.yml` and `Dockerfile` in the repo.
2. Add `templates/` directory with the config/sample/report templates as files (the README references them).
3. Add example command-line invocations for the workflow engine you use and minimal test data in `tests/` for CI.
4. Fill in QC thresholds based on validation runs and update `docs/qc.md`.
