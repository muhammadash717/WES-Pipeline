...
try:
    ...
    output_doc = os.path.dirname(input_tsv) + f"./{patient_name} {patient_id} WES Report.docx"
...
doc.add_heading("Single Nucleotide Variants and Small INDELs", level=1)
...
if len(primary_variants) > 0:
    ...
    hdr_cells[2].text = 'Zygosity'
    hdr_cells[3].text = 'Class'
    hdr_cells[4].text = 'Disease'

    for _, row in pd.concat([primary_variants, empty_row], ignore_index=True).iterrows():
        r = table.add_row().cells
        gene = str(row.get("Gene"))
        gene_omim_id = str(row.get("OMIM Gene ID"))

        r[0].text = gene + "\n(" + str(row.get("Transcript ID")).split(".")[0] + ")"
        r[1].text = str(row.get("hgvsc")) + ("; " + str(row.get("hgvsp")) if row.get("hgvsp") else "")
        r[2].text = str(row.get(f"Zygosity"))
        r[3].text = ' '.join(re.findall(r'[A-Z][a-z]*', str(row.get("Franklin ACMG Class"))))

        r[4].text = "- " + str(row.get("OMIM")).replace(" | ", "\n- ") + "\n"
        add_hyperlink(r[4].paragraphs[-1], f"https://www.omim.org/entry/{gene_omim_id}", f"{gene} - OMIM: {gene_omim_id}")
...
if len(primary_variants) > 0:
    ...
    for _, row in primary_variants.iterrows():
        ...
        variant_type = str(row.get("Franklin Effect", ""))
        hgvsc = str(row.get("hgvsc", ""))
        hgvsp = str(row.get("hgvsp", ""))
        zygosity = str(row.get(f"Zygosity", ""))
        # exon_rank = str(row.get("exon_rank", ""))
        # exon_count = str(row.get("exon_count", ""))
        # intron_rank = str(row.get("intron_rank", ""))
        # amino_acid = f'{str(row.get("aa_start", ""))}/{str(row.get("aa_length", ""))}'

        # if len(exon_rank) > 0:
        #     segment = 'exon'
        #     segment_number = f'{int(exon_rank.rstrip(".0"))}/{exon_count}'
        # else:
        #     segment = 'intron'
        #     segment_number = f'{int(intron_rank.rstrip(".0"))}/{int(exon_count)-1}'

        transcript_id = str(row.get("Transcript ID", ""))
        gene = str(row.get("Gene", ""))
        disease = str(row.get("OMIM", ""))
        acmg = str(row.get("Franklin ACMG Class", ""))
        clinvar_class = str(row.get("ClinVar Classification", ""))
        hgnc_id = str(row.get("HGNC ID", ""))
        gene_description = str(row.get("Gene Description", ""))
        gene_omim_id = str(row.get("OMIM Gene ID", ""))
        disease_omim_ids = str(row.get("OMIM Phenotype IDs", ""))
        dbsnp = str(row.get("dbSNP"))
        diseases_description = str(row.get("Disease Summary", ""))

        variant_paragraph = f"The {variant_type} variant {hgvsc}, is found in {zygosity} state in transcript {transcript_id} of the {gene} gene. "

        if len(hgvsp) > 0:
            variant_paragraph = variant_paragraph + f"The protein-level change is {hgvsp}. "

        variant_paragraph = variant_paragraph + f"In silico prediction algorithms estimate that this variant would have the following effects: "

        computational_sources = ["REVEL", "AlphaMissense", "BayesDel NoAF", "Phylop 100way", "SpliceAI Max", "DbscSNV ADA", "Apogee2", "MitoTIP"]
        for tool in computational_sources:
            score = str(row.get(tool+" Score", ""))
            if len(score) > 0:
                pred = str(row.get(f"{tool} Prediction"))
                variant_paragraph = variant_paragraph + f"{pred} ({tool} = {score}), "

        variant_paragraph = variant_paragraph[:-2] + ". "

        if len(clinvar_class) > 0:
            variant_paragraph = variant_paragraph + f"In the NCBI ClinVar database, it is described as a \"{clinvar_class}\" variant. "
        else:
            variant_paragraph = variant_paragraph + "In the NCBI ClinVar database, it has not been described. "
        
        if dbsnp != "nan":
            variant_paragraph = variant_paragraph + f'It has the dbSNP accession number {dbsnp}. '
        else:
            variant_paragraph = variant_paragraph + "It does not have a dbSNP accession number. "
        
        gnomad_exomes_af = str(row.get("GnomAD Exomes AF"))
        gnomad_genomes_af = str(row.get("GnomAD Genomes AF"))
        if len(gnomad_genomes_af) > 0 or len(gnomad_exomes_af) > 0:
            variant_paragraph = variant_paragraph + f"Finally, it has been identified in gnomAD with frequency (AF): gnomAD Exome={gnomad_exomes_af or None} and gnomAD Genome={gnomad_genomes_af or None}. "
        else:
            variant_paragraph = variant_paragraph + "Finally, it has not been identified in gnomAD. "
        
        variant_paragraph = variant_paragraph + f"With this information, we classify this change as {' '.join(re.findall(r'[A-Z][a-z]*', acmg))}."
        
        doc.add_heading(f"{hgvsc} in the gene {gene}", level=2)
        ...
        doc.add_paragraph(f"** NCBI Summary for the gene {gene}: {gene_description}")
...
if len(attached_variants) > 0:
    ...
    for _, row in attached_variants.iterrows():
        ...
        computational_sources = ["AlphaMissense", "Apogee2", "BayesDel NoAF", "DbscSNV ADA", "MitoTIP", "Phylop 100way", "REVEL", "SpliceAI Max"]
        for tool in computational_sources:
            score = str(row.get(tool+" Score", ""))
            if len(score) > 0:
                pred = str(row.get(f"{tool} Prediction"))
